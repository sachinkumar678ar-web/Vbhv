...
let lastMsgId = null
let lastChance = ""
let predictedIssue = "" // Jis period ke liye prediction di gayi hai

// ---------------- RULE ENGINE (Based on Next Period's Last Digit) ----------------
function ruleEngine(nextPeriodNumber){
    const lastDigit = parseInt(nextPeriodNumber.toString().slice(-1));
    const rules = {
...
// ---------------- RESULT CHECK ----------------
async function checkResult(number, currentIssue){
    // Sirf tab check karega jab currentIssue wahi ho jiska humne prediction diya tha
    if(predictedIssue !== currentIssue) return;
    ...
    if(suggestedNumbers.includes(number)){
        resultStatus = "🤩 JACKPOT"; sticker = JACKPOT_STICKER;
    } else if((lastPrediction=="BIG" && number>=5) || (lastPrediction=="SMALL" && number<=4)){
        resultStatus = "✅ WIN"; sticker = WIN_STICKER;
    } else {
        resultStatus = "❌ LOSS"; sticker = LOSS_STICKER;
    }
...
// ---------------- PREDICTION SEND ----------------
async function sendPrediction(currentIssue){
    const nextIssue = (BigInt(currentIssue) + 1n).toString();
    const rule = ruleEngine(nextIssue); // Agle period ke last digit par prediction

    lastPrediction = rule.result;
    suggestedNumbers = rule.nums;
    lastChance = rule.chance;
    predictedIssue = nextIssue; // Save karo ki kis period ke liye bet di hai
...
async function scan(){
    try {
        ...
        // 1. Pehle Result Check karo (Kya pichli prediction sahi thi?)
        await checkResult(number, issue)

        lastIssue = issue;

        // 2. History Channel Update
...
━━━━━━━━━━━━━━`)
        ...
        // 3. Ab Agle Period ke liye prediction do
