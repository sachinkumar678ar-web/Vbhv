...
async function sendPrediction(number,issue){

const rule = ruleEngine(number)

lastPrediction = rule.result
suggestedNumbers = rule.nums
lastChance = rule.chance

const shortIssue = issue.slice(-4)
...
const issue = list[0].issue
const number = String(list[0].number)
...
if(lastIssue!=""){
await checkResult(Number(number))
}
